...
func wrapGenericHandler(core *vault.Core, h http.Handler, props *vault.HandlerProperties) http.Handler {
	...
	var maxJSONDepth, maxStringValueLength, maxObjectEntryCount, maxArrayElementCount int64
	...
	return http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		// Validate JSON limits for the request body
		...
		// Validate JSON limits early if this is a JSON request
		buf, err := validateJSONLimits(w, r, maxRequestSize, jsonLimits)
		if err != nil {
...
// multiReaderCloser combines multiple ReadClosers into a single ReadCloser
type multiReaderCloser struct {
	io.Reader
	closers []io.Closer
}

func newMultiReaderCloser(buf *bytes.Buffer, origBody io.ReadCloser) *multiReaderCloser {
	return &multiReaderCloser{
		Reader:  io.MultiReader(buf, origBody),
		closers: []io.Closer{origBody},
	}
}
...
func validateJSONLimits(w http.ResponseWriter, r *http.Request, maxRequestSize int64, jsonLimits jsonutil.JSONLimits) (*bytes.Buffer, error) {
